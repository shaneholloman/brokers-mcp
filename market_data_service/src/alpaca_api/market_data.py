...
stock_client = StockHistoricalDataClient(settings.api_key, settings.api_secret)
...
def bars_back_to_datetime(unit: str, bar_size: int, bars_back: int, extended_hours=False) -> datetime:
...
async def get_alpaca_bars(
    ...
    # Get the bars
    bars_df = stock_client.get_stock_bars(request).df
...
async def plot_alpaca_bars_with_indicators(
    ...
    return (
        ...
        bars_df.iloc[-70:].loc[:, ['datetime', 'open', 'low', 'high', 'close', 'volume', 'vwap']].to_json(orient="records", lines=True)
    ) 

def get_bars_resource(symbol: str, unit: str, bar_size: int, indicators: str = "", bars_back: Optional[int] = None, extended_hours: bool = False) -> str:
    return get_alpaca_bars(symbol, unit, bar_size, indicators, bars_back, extended_hours)
