...
def endpoint_uri(protocol: str, endpoint: str) -> AnyUrl:
    return AnyUrl(f"{protocol}://{endpoint}")


resource_list = [
    Resource(
        uri=endpoint_uri(protocol="account", endpoint="portfolio"),
        name="portfolio",
        description="account portfolio holdings, including stocks, options, and futures",
        mimeType="plain/text",
    ),
    Resource(
        uri=endpoint_uri(protocol="account", endpoint="account_summary"),
